...
stock::stock(int IDS, int QUANTS, QString NOMS, QString CATEGORIE,
             QString FOURNIS, QString CONDS, QDate DATES, QDate DATEAS, int idStock) {
...
// Méthode pour ajouter un enregistrement dans la base de données
    bool stock::ajouter() {
        ...
        query.prepare("insert into STOCK (IDS, NOMS,  CATEGORIE, QUANTS, FOURNIS, CONDS,DATES, DATEAS) "
                      "values (:IDS, :NOMS, :CATEGORIE, :QUANTS, :FOURNIS, :CONDS, :DATES, :DATEAS)");
...
/*
QMap<QString, double> stock::calculStat() {
    QMap<QString, double> statistiques;
    double totalQuantites = 0.0;

    // Récupérer les quantités et catégories depuis la base de données
    QSqlQuery query("SELECT CATEGORIE, SUM(QUANTS) as totalQuantite FROM stock GROUP BY CATEGORIE");
    ...
    while (query.next()) {
        ...
        double quantite = query.value("totalQuantite").toDouble();
        statistiques[categorie] = quantite;
        totalQuantites += quantite;
    }

    // Convertir les quantités en pourcentages
    for (auto it = statistiques.begin(); it != statistiques.end(); ++it) {
        statistiques[it.key()] = (it.value() / totalQuantites) * 100.0;
    }
