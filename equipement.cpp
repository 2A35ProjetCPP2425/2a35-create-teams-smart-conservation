...
QSqlQueryModel* equipement::rechercherpanom(const QString &input) {
    ...
    query.prepare("SELECT * FROM MEHDI.EQUIPEMENT WHERE TO_CHAR(ID_EQUIPEMENT) LIKE :partialId OR NOM LIKE :nom");
...
QString equipement::diagnostic(int id) {
    ...
    if (!query.prepare("SELECT PARAMETRE_SUIVI FROM MEHDI.EQUIPEMENT WHERE ID_EQUIPEMENT = :id")) {
